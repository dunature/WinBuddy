...
describe('SubmissionService', () => {
  test('上传包保持在 quarantine，hash 正确后进入校验', async () => {
    ...
    expect(created.objectKey).toContain('quarantine/submissions/')
    const body = new Uint8Array([1, 2, 3])
    await objects.putPackage(created.objectKey, body)
