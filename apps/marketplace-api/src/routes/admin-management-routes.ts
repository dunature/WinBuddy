...
export function createAdminManagementRoutes(auth: AdminAuthService, service: AdminManagementService): Hono<ManagementEnv> {
  ...
  routes.post('/skills/:id/lifecycle', async (context) => { const user = context.get('managementUser'); if (user.role !== 'admin') return accessDenied(context); const body = await context.req.json() as { action?: string; reason?: string }; if (!['unlist', 'republish', 'archive'].includes(body.action ?? '')) return context.json({ code: 'VALIDATION_FAILED', message: '生命周期操作无效', requestId: context.get('requestId') }, 400); try { await service.lifecycle(context.req.param('id'), body.action as 'unlist' | 'republish' | 'archive', body.reason ?? '', user.githubLogin); return context.json({ ok: true }) } catch (error) { if (error instanceof SubmissionError) return context.json({ code: error.code, message: error.message, requestId: context.get('requestId') }, error.status as 400); throw error } })
  routes.get('/audit', async (context) => context.get('managementUser').role === 'admin' ? context.json(await service.audit(context.req.query('skillId'))) : accessDenied(context))
  return routes
}

function accessDenied(context: Context<ManagementEnv>) {
  return context.json({ code: 'ADMIN_ACCESS_DENIED', message: '只有 Admin 可以执行该操作', requestId: context.get('requestId') }, 403)
}
